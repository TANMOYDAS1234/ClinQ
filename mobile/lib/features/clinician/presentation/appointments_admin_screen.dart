...
class AppointmentsAdminScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AppointmentsAdminScreen> createState() => _AppointmentsAdminScreenState();
}

class _AppointmentsAdminScreenState extends ConsumerState<AppointmentsAdminScreen> {
  ...
  AppointmentQuery get _query {
    switch (_scope) {
      ...
      case 'upcoming':
        return (from: todayBounds().from, to: null, status: _status, clinicId: null);
      default:
    ...
    return Scaffold(
      ...
      body: AutoRefresh(
        ...
        child: RefreshIndicator(
          onRefresh: () async => ref.invalidate(appointmentDiaryProvider(_query)),
          child: async.when(
            ...
            error: (_, _) => Center(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: AppSpacing.sm),
                  OutlinedButton(onPressed: () => ref.invalidate(appointmentDiaryProvider(_query)), child: const Text('Retry')),
                ],
              ),
            ),
            data: (paged) {
              final items = [...paged.items]..sort((a, b) => _ascending
              ...
              if (items.isEmpty) {
                return ListView(
                  children: [
                    ...
                    Icon(Icons.event_busy_outlined, size: 56, color: scheme.outlineVariant),
                    const SizedBox(height: AppSpacing.md),
                    const Center(child: Text('No appointments', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600))),
                  ],
                );
              }
  ...
  Future<void> _manage(Appointment a) async {
    await showModalBottomSheet<void>(
      ...
      builder: (_) => _ManageSheet(
        appointment: a,
        onAction: (targetStatus, {String? notes, bool cancel = false}) async {
          final messenger = ScaffoldMessenger.of(context);
          Navigator.pop(context);
          try {
            if (cancel) {
              await ref.read(appointmentRepositoryProvider).cancel(a.id, reason: notes);
            } else {
              await ref.read(appointmentRepositoryProvider).setStatus(a.id, targetStatus!, consultationNotes: notes);
            }
...
typedef _ActionCallback = Future<void> Function(String? targetStatus, {String? notes, bool cancel});
...
class _ManageSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    switch (a.status) {
      case 'requested':
        actions.add(_Action('Confirm', Icons.check_circle_outline_rounded, AppColors.success, status: 'confirmed'));
        actions.add(_Action('Mark no-show', Icons.person_off_outlined, AppColors.warning, status: 'no_show'));
      case 'confirmed':
        actions.add(_Action('Check in', Icons.login_rounded, AppColors.primary, status: 'checked_in'));
        actions.add(_Action('Start consultation', Icons.play_circle_outline_rounded, AppColors.primary, status: 'in_consultation'));
        actions.add(_Action('Mark no-show', Icons.person_off_outlined, AppColors.warning, status: 'no_show'));
      case 'checked_in':
        actions.add(_Action('Start consultation', Icons.play_circle_outline_rounded, AppColors.primary, status: 'in_consultation'));
      case 'in_consultation':
        actions.add(_Action('Complete', Icons.task_alt_rounded, AppColors.success, status: 'completed', notes: true));
    }
          ...
          children: [
            ...
            if (actions.isEmpty && !canCancel)
              Padding(
                ...
                child: Text('No actions available for a ${a.status} appointment.',
    ...
    final notes = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          maxLines: 4,
          decoration: const InputDecoration(hintText: 'Optional notes for the record'),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Skip')),
          FilledButton(onPressed: () => Navigator.pop(ctx, controller.text.trim()), child: const Text('Complete')),
        ],
      ),
    );
...
class _Action {
  const _Action(this.label, this.icon, this.color, {this.status, this.notes = false});
  final String label;
  final IconData icon;
  final Color color;
  final String? status;
